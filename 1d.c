#include <fftw3.h> 
#define NUM_POINTS 8192  // input N
#define runs 10000 
...
void acquire_signal(fftw_complex* signal) { 
 // Generates sine waves of different frequencies and amplitudes. 
	...
	long double theta = (long double)i / (long double)NUM_POINTS * M_PI; 
...
int main() { 
	...
	mflops = (5*NUM_POINTS * (log(NUM_POINTS)/log(2)))/(time_spent*(1000000/runs));
	printf("MFLOPS = %f\n",mflops);
	return 0; 
}
